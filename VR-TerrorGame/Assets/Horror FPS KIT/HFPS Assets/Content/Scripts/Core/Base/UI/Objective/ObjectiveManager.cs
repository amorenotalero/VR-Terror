...
namespace HFPS.Systems
{
            ...
            foreach (var obj in SceneObjectives.Objectives)
            {
                if (obj.objectiveID == model.identifier)
                {
                    ...
                    foreach (var evt in GetEvents())
                    {
                        if (evt.EventID.Equals(obj.eventID))
                        {
                            return new ObjectiveModel(model.identifier, model.toComplete, model.isCompleted, evt)
                            {
    ...
    public sealed class ObjectiveModel
    {
        ...
        public ObjectiveModel(int id, int count, string text, ObjectiveEvent evt)
        {
            identifier = id;
            toComplete = count;
            objectiveText = text;
        ...
        public ObjectiveModel(int id, int count, bool completed, ObjectiveEvent evt)
        {
            identifier = id;
            toComplete = count;
