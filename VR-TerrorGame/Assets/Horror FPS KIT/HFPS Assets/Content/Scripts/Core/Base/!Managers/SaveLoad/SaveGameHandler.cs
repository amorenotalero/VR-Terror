    ...
    public class SaveGameHandler : Singleton<SaveGameHandler>
    {
        ...
        async void Awake()
        {
            ...
            if (Prefs.Exist(Prefs.LOAD_STATE))
            {
                ...
                if (loadstate == 0)
                {
                    ...
                    inventory.IntItemOnStart();
                }
                else if (loadstate == 1 && Prefs.Exist(Prefs.LOAD_SAVE_NAME))
                {
                    string filename = Prefs.Game_SaveName();

                    if (File.Exists(Path.Combine(jsonManager.GetCurrentPath(), filename)))
                    {
        ...
        void LoadSerializedData(JObject json = null)
        {
            ...
            if (json.ContainsKey("objectivesData"))
            {
                ...
                {
                    Dictionary<int, Dictionary<string, string>> activeObjectives = jsonManager.Json<Dictionary<int, Dictionary<string, string>>>(json["objectivesData"]["activeObjectives"].ToString());
        ...
        private IEnumerator DeserializeInventory(JToken token)
        {
            ...
            for (int i = 0; i < inventory.Slots.Count; i++)
            {
                ...
                if (slotString != "null")
                {
                    inventory.AddItemToSlot((int)slotToken["slotID"], (int)slotToken["itemID"], (int)slotToken["itemAmount"], slotToken["itemData"].ToObject<ItemData>());
                }
            }
