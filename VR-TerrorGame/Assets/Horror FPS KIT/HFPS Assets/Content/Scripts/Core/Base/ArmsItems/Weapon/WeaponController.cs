...
namespace HFPS.Player
{
    ...
    public class WeaponController : SwitcherBehaviour, ISaveableArmsItem, IOnAnimatorState
    {
        ...
        [Serializable]
        public sealed class ShotgunSettings
        {
        ...
        public ShotgunSettings shotgunSettings = new ShotgunSettings();
        #endregion
        ...
        void Awake()
        {
            ...
            playerFunctions = scriptManager.Get<PlayerFunctions>();
        ...
        void FireOneBullet()
        {
            ...
            kickback.ApplyKickback(new Vector3(
                kickbackSettings.kickUp,
        ...
        {
            if (weaponType == WeaponType.Shotgun && shotgunSettings.shellPrefab && shotgunSettings.ejectPosition)
            {
                GameObject shell = Instantiate(shotgunSettings.shellPrefab, shotgunSettings.ejectPosition.position, Quaternion.Euler(shotgunSettings.ejectPosition.eulerAngles + shotgunSettings.shellRotation));
                Physics.IgnoreCollision(shell.GetComponent<Collider>(), Player.GetComponent<CharacterController>());

                //Vector3 playerVelocity = Player.GetComponent<CharacterController>().velocity.normalized * 2f;
                shell.GetComponent<Rigidbody>().AddForce(shotgunSettings.ejectPosition.transform.right * shotgunSettings.ejectSpeed);
            }
        }
