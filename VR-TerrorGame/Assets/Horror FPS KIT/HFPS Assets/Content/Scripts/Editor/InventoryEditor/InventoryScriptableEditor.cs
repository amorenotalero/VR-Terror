...
namespace HFPS.Editors
{
    ...
    public class InventoryScriptableEditor : Editor
    {
        ...
        public static bool OnOpenAsset(int instanceId, int line)
        {
            ...
            if(obj != null && obj is InventoryScriptable)
            {
                OpenDatabaseEditor();
                return true;
            }
        ...
        static void OpenDatabaseEditor()
        {
            if (Target != null)
            {
                InventoryEditorWindow itemsEditor = EditorWindow.GetWindow<InventoryEditorWindow>(false, Target.name, true);

                itemsEditor.minSize = new Vector2(800, 450);
                itemsEditor.Initialize(Target);
            }
            else
            {
        ...
        public static void OpenDatabaseEditor(InventoryScriptable database)
        {
            if (database != null)
            {
                ...
                itemsEditor.minSize = new Vector2(800, 450);
                itemsEditor.Initialize(database);
            }
            else
            {
